package com.comcast.apps.dcmFeature.restapi;
...
public class DcmFeatureControlSettingsRestHandler {
	...
	public Response getDcmSettings(@Context HttpServletRequest httpServletRequest,
		...
		try {
			...
			String httpQuery = "https://xconf.xcal.tv/featureControl/getSettings?" + httpServletRequest.getQueryString();
			LOGGER.info("About to send the request " + httpQuery);
			...
			String dcmConfiguration = response.getEntity(String.class);
			LOGGER.info("The response obtained is : " + dcmConfiguration);

			LOGGER.info("[Performance Metrics] Time taken to complete one request in real Setup using CI XCONF = "
					+ (startTime - startTimeProdXconf));
					...
					if (DATA_STORE_UPLOAD_SETTINGS.containsKey(estbMacAddress)) {
						...
						if (null != dcmFeatureControlSettings && null != dcmConfiguration) {
							...
							if (null != dcmFeatureSettings) {
								modifyUploadDcmFeatureControlSettingsFromProdXconf(xconfDcmSetting, dcmFeatureSettings);	
							}
	 ...
	 */
	private void modifyUploadDcmFeatureControlSettingsFromProdXconf(JSONObject xconfDcmSetting, JSONArray dcmFeatureSettings)
			throws JSONException {
